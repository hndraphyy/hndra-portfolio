...
const MenuOverlay = ({ isAnimating, isOpen, onToggle }: MenuOverlayProps) => {
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      <div
        className={`absolute inset-0 bg-white text-black transform transition-all duration-300 ease-in-out px-4 lg:px-8 py-6 ${
          isAnimating ? "animate-slideUp" : isOpen ? "animate-slideDown" : ""
        }`}
      >
        <button
          onClick={onToggle}
          className="absolute px-4 sm:px-6 lg:px-8 right-0 top-4 text-sm md:text-xl font-extralight text-black"
        >
          Menu
        </button>

        <ul className="text-2xl md:text-4xl lg:text-5xl font-extralight space-y-3 md:space-y-4 lg:space-y-6">
          {navItems.map((item) => (
